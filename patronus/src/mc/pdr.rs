...
impl BasePdr {
    ...
    ///
    /// # Errors
    /// Returns [`UnexpectedResponse`] if any SMT query returns `UNKNOWN` or original cube
    ...
    ///
    /// # Errors
    /// In cases of `Unknown` SMT queries, return [`UnexpectedResponse`]
    fn get_bad_cube(
        &self,
        ctx: &mut Context,
        smt_ctx: &mut impl SolverContext,
    ...
    fn block_cube(
        ...
        while let Some(obj) = worklist.pop() {
            ...
            let obj_from = expr_at_step(ctx, enc, obj_expr, FROM_STEP);
            if self

...
use patronus::expr::*;
use patronus::smt::{Solver, solver_from_env};

#[derive(Default)]
struct Tester {
    ctx: Context,
    simplifier: DefaultSimplifier,
}
...
impl Drop for Tester {
    fn drop(&mut self) {
        // try to verify all rewrites
        let mut smt_ctx = solver_from_env().start(None).unwrap();
        self.simplifier
            .verify_simplification(&mut self.ctx, &mut smt_ctx)
            .unwrap();
    }
}
...
fn test_conditional_simplification() {
    ...
    // a = 0 -> a + 1 = 0 + 1 = 1
    // a = 1 -> a     = 1
    t.ts("ite(not(a:bv<1>), add(a, 1'b1), a)", "true");
}
...
fn test_simplify_add() {
    ...
    t.ts("add(a : bv<1>, b: bv<1>)", "xor(a : bv<1>, b: bv<1>)");
    t.ts("add(a : bv<1>, 1'b1)", "not(a : bv<1>)");
    t.ts("add(a : bv<1>, 1'b0)", "a : bv<1>");
}
...
fn test_simplify_mul() {
    ...
    t.ts("mul(a : bv<1>, 1'b1)", "a : bv<1>");
    t.ts("mul(a : bv<1>, 1'b0)", "1'b0");

    // canonicalize multiply 1-bit
    t.ts("mul(a : bv<1>, b: bv<1>)", "and(a : bv<1>, b: bv<1>)");
}
...
fn test_simplify_implies() {
    ...
    t.ts("implies(a : bv<1>, b : bv<1>)", "or(not(a:bv<1>), b:bv<1>)");
}
...
fn test_simplify_ugte() {
    ...
    t.ts("ugte(a : bv<1>, 1'b1)", "a : bv<1>"); // here `a` is either 0 or 1
    t.ts("ugte(a : bv<32>, 32'd0)", "true");
    t.ts(
